...
class CountingStateStore:
    ...
    the DamageHandler's own save() call count without instrumenting the
...
def test_damage_handler_persists_hp_through_real_filesystem_state_store(
    tmp_path: Path,
) -> None:
    ...
    command = ApplyDamageCommand(
        ...
        payload=ApplyDamagePayload(target_id="monster_001", amount=3),
    )
    ...
    # reload through FilesystemStateStore reflects the new HP
    ...
    assert reloaded_target.current_hp == 4
    assert reloaded_target.max_hp == 7
    ...
    assert (reloaded_other.current_hp, reloaded_other.max_hp) == (7, 7)
