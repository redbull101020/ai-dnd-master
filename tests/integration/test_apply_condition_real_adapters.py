...
FIXED_TIMESTAMP = datetime(2026, 8, 28, 19, 0, tzinfo=timezone.utc)


class CountingStateStore:
    """Thin call-counting wrapper around a real StateStore, for observing
    the ApplyConditionHandler's own save() call count without instrumenting
...
def test_apply_condition_handler_persists_through_real_filesystem_state_store(
    tmp_path: Path,
) -> None:
    ...
    assert serialized_target["conditions"] == ["poisoned"]
    ...
    assert reloaded_target.conditions == frozenset({Condition.POISONED})
    ...
    assert reloaded_other.conditions == frozenset()
