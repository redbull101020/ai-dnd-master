...
from dnd_engine.infrastructure.persistence.json.state_serializer import SCHEMA_VERSION
from dnd_engine.infrastructure.random.dice import PythonDiceEngine


FIXED_TIMESTAMP = datetime(2026, 8, 27, 12, 0, tzinfo=timezone.utc)
...
def test_skill_check_uses_real_adapters_without_persisting_artifacts(
    tmp_path: Path,
) -> None:
    ...
    assert f'"schemaVersion": {SCHEMA_VERSION}'.encode() in state_before
    ...
    seed = 20260827
    result = SkillCheckHandler(
        state_store=store,
        dice=PythonDiceEngine(random.Random(seed)),
        event_metadata_provider=metadata,
    ).handle(command)
    repeated = SkillCheckHandler(
        state_store=store,
        dice=PythonDiceEngine(random.Random(seed)),
