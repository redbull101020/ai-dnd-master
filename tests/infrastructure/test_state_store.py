...
def test_save_load_v7_preserves_weapon_source_state(tmp_path: Path) -> None:
    """Real filesystem round-trip evidence for TSK-0004 (weapon-source
    State, now written as V7): proves non-empty weapon_proficiencies,
    InventoryState, InventoryItemState, and EquipmentState survive an exact
    Domain round-trip through actual on-disk JSON, not just an in-memory
    fake."""
