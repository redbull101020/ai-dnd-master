...
def make_command() -> RemoveConditionCommand:
...
def test_remove_condition_command_has_exact_fields_and_fixed_type() -> None:
    command = make_command()

    assert tuple(field.name for field in fields(RemoveConditionPayload)) == (
        "target_id",
        "condition",
    )
    assert tuple(field.name for field in fields(RemoveConditionCommand)) == (
        "command_id",
        "campaign_id",
        "actor_id",
        "payload",
    ...
    assert command.type == "RemoveConditionCommand"
...
def test_remove_condition_command_and_payload_are_immutable() -> None:
    command = make_command()
...
def test_payload_requires_exact_string_target_id(target_id: object) -> None:
    with pytest.raises(TypeError):
        RemoveConditionPayload(  # type: ignore[arg-type]
...
def test_command_requires_exact_string_ids(field_name: str) -> None:
    ...
    with pytest.raises(TypeError):
        RemoveConditionCommand(**values)  # type: ignore[arg-type]
