...
from dnd_engine.domain.rules.remove_condition import (
...
def canonical_result(**overrides: object) -> ConditionRemovalResult:
    ...
    return ConditionRemovalResult(**values)  # type: ignore[arg-type]


# --- resolve_condition_removal: gameplay behaviour -----------------------


def test_removing_active_condition_becomes_absent() -> None:
    result = resolve_condition_removal(
        make_command(), make_creature(conditions=frozenset({Condition.POISONED}))
    )

    assert result == ConditionRemovalResult(
        target_id="monster_001",
        condition=Condition.POISONED,
        previous_active=True,
        active=False,
...
def test_removing_already_absent_condition_is_a_successful_no_op() -> None:
    result = resolve_condition_removal(
...
def test_resolver_does_not_mutate_target() -> None:
    target = make_creature(conditions=frozenset({Condition.POISONED}))
    target_before = deepcopy(target)

    resolve_condition_removal(make_command(), target)
...
def test_same_command_and_target_produce_same_result() -> None:
    ...
    first = resolve_condition_removal(command, target)
    second = resolve_condition_removal(command, target)
...
@pytest.mark.parametrize(
    ...
    [
        (object(), make_creature(), "RemoveConditionCommand"),
...
def test_resolver_rejects_wrong_domain_input_types(
    ...
    with pytest.raises(TypeError, match=match):
        resolve_condition_removal(command, target)  # type: ignore[arg-type]


# --- ConditionRemovalResult: shape and invariants -------------------------


def test_condition_removal_result_has_exact_fields() -> None:
    assert tuple(field.name for field in fields(ConditionRemovalResult)) == (
        "target_id",
        "condition",
        "previous_active",
        "active",
...
def test_condition_removal_result_is_immutable() -> None:
    result = canonical_result()
...
def test_condition_removal_result_rejects_wrong_runtime_types(
    field_name: str,
...
def test_condition_removal_result_requires_active_false() -> None:
    with pytest.raises(ValueError, match="active"):
        canonical_result(active=True)
