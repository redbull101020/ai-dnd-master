...
def make_command() -> EndCombatCommand:
...
def test_end_combat_command_has_exact_fields_and_fixed_type() -> None:
    command = make_command()

    assert tuple(field.name for field in fields(EndCombatPayload)) == (
        "combat_id",
    )
    assert tuple(field.name for field in fields(EndCombatCommand)) == (
        "command_id",
        "campaign_id",
        "actor_id",
        "payload",
    ...
    assert command.type == "EndCombatCommand"


def test_end_combat_command_and_payload_are_immutable() -> None:
    command = make_command()
...
def test_payload_requires_exact_string_combat_id(combat_id: object) -> None:
    with pytest.raises(TypeError):
        EndCombatPayload(combat_id=combat_id)  # type: ignore[arg-type]
...
def test_command_requires_exact_string_ids(field_name: str) -> None:
    ...
    with pytest.raises(TypeError):
        EndCombatCommand(**values)  # type: ignore[arg-type]
