...
FIXED_TIMESTAMP = datetime(2026, 8, 28, 18, 0, tzinfo=timezone.utc)
...
def test_successful_condition_application_follows_canonical_lifecycle_and_persists() -> (
    None
):
    actor = make_actor()
    ...
    assert target.conditions == frozenset()
    ...
    assert saved_target.conditions == frozenset({Condition.POISONED})
...
def test_successful_no_op_application_when_already_active_still_persists() -> None:
    actor = make_actor()
...
def test_resolver_failure_prevents_save(
    ...
    import dnd_engine.application.handlers.apply_condition as handler_module
    ...
    monkeypatch.setattr(
        handler_module, "resolve_condition_application", failing_resolver
    )
...
def test_event_builder_failure_prevents_save(
    ...
    monkeypatch.setattr(
        handler_module, "build_condition_applied_v1", failing_builder
    )
...
def test_applier_invariant_failure_prevents_save(
    ...
    monkeypatch.setattr(
        handler_module, "apply_condition_applied_v1", failing_applier
    )
