...
def build_ability_check_resolved_v1(
    *,
    ...
    if outcome.roll.mode is not RollMode.NORMAL:
        raise ValueError("AbilityCheckResolved V1 supports only normal rolls")

    legacy_roll = DiceRoll(
        expression="1d20",
        rolls=outcome.roll.rolls,
        total=outcome.roll.selected,
    )
