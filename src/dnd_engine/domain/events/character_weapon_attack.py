...
class CharacterWeaponAttackResolvedPayloadV1:
    target_id: str
    weapon_item_id: str
...
def build_character_weapon_attack_resolved_v1(
    *,
    event_id: str,
    timestamp: datetime,
    ...
    if type(weapon_item_id) is not str:
        raise TypeError("weapon_item_id must be a str")
    if type(weapon_definition_id) is not str:
        raise TypeError("weapon_definition_id must be a str")
    ...
    if command.payload.weapon_item_id != weapon_item_id:
        raise ValueError(
            "weapon_item_id must match command payload weapon_item_id"
        )

    payload = CharacterWeaponAttackResolvedPayloadV1(
    ...
    return GameEvent(
        ...
        type="CharacterWeaponAttackResolved",
        version=1,
