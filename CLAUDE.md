...
`HealingHandler`. Shared snapshot-replacement helper между четырьмя handlers
намеренно не извлечён: список выше остаётся в силе, и оценка этой duplication —
задача отдельной, специально выделенной abstraction-review группы. EventStore,
...
* Current `state.json` содержит целочисленный `schemaVersion = 4`; writer выпускает только V4, reader принимает exact V4 и legacy V1/V2/V3. V1 мигрирует в `characters=()`, V2 — в empty `skill_proficiencies`, V1–V3 — в empty `CreatureState.conditions`. Это версия схемы, а не ревизия State.
