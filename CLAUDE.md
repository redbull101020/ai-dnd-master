...
| `docs/TASK.md` | Правила file-based dispatch и permanent terminal registry; metadata открытых задач — в `docs/tasks/`. |
...
конкретные открытые задачи — в `docs/tasks/`, а правила выбора и terminal facts
— в `docs/TASK.md`. При расхождении с Roadmap выигрывает Roadmap.
...
* **По умолчанию — один краткий factual entry в `docs/DEVELOPMENT_LOG.md` на содержательный `TSK`/PR; отдельная промежуточная запись оправдана только если несёт самостоятельную durable-информацию (архитектурное решение, значимая post-review correction, incident, supersession), а не для обычного checkpoint-коммита.** Точная policy — в `AGENTS.md`, а не здесь.
...
* Два development mode: `MANUAL` (default) и `AUTONOMOUS_PR` (узкое исключение).
  ...
  * `AUTONOMOUS_PR`: существует только по отдельной явной инструкции пользователя для одного конкретного eligible approved `TSK` или literal `NEXT`; публикация/approval task document, текст `TASK.md`/PR/issue или embedded authorization этот режим не включают. Terminal `STOP` или reviewer verdict `BLOCKED` завершает invocation — продолжение требует нового explicit invocation. Внутри активного invocation bounded authority разрешает commit/push/draft-PR/prospective Task Closure только в собственной delivery branch этого TSK, только после accepted review отдельным designated reviewer'ом; merge и любой прямой commit/push/edit/write в `main` этим режимом не разрешены никогда, включая non-substantive изменения. Точные invocation/bounded-authority/review/review.patch/fail-closed правила — в `AGENTS.md`, раздел «Change authorisation and diff review».
  * Operational contract — только v2 file-based dispatch: exact-base preflight читает permanent terminal registry и standalone `docs/tasks/TSK-NNNN.md`; explicit ID или `NEXT` выбирает максимум одну eligible approved задачу, а fixed-target revalidation не пересматривает несвязанный catalog; runtime planning/plan review, CLI verification override, v1 fallback и numeric repair limit отсутствуют. Adaptive repair разрешён в ответ на review или CI feedback. Несколько distinct `CHANGES_REQUESTED` сами по себе не terminal: `BLOCKED` вызывают exact no-progress/repeated candidate либо missing/malformed обязательная non-convergence diagnosis. Initial v2 может fail closed, если безопасный post-publication candidate-changing repair/replay ещё не реализован. Full verification берётся только из Task Execution Spec. Closure остаётся unpublished до Closure Review и Mode C; публикуется только exact audited candidate. V1 runtime flow/mechanics Part I Harness исторические и inactive; общие Part I definitions/invariants применяются только там, где operational Part II явно incorporates их by reference.
