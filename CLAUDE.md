...
* Application orchestration (§2.2, §§3.10, 3.13–3.14): `EventMetadata`, `EventMetadataProvider`, explicit `AbilityCheckHandler`, `SavingThrowHandler` и `SkillCheckHandler`. Read-only handlers загружают State, находят нужные projections, вызывают concrete resolver, получают metadata через injected provider и собирают Event/`ResolutionResult`; они не читают clock, не генерируют ID, не мутируют State и не вызывают `StateStore.save()`.
