...
* State Mutation Foundation (G5, §3.18) зафиксирован как canonical contract для будущего authoritative state-mutating Command; первый конкретный consumer этого контракта реализован — minimal Damage → HP mutation slice (G6A, §3.19): `ApplyDamageCommand` → `DamageApplied` V1 → `CreatureState.current_hp` replacement → `StateStore.save()`. Broad Roadmap `HP`/`Damage`/`Healing` остаются incomplete.
...
exact MVP atomicity boundary — зафиксирован в §3.18. Один конкретный Event
applier и один конкретный mutating handler реализованы для Damage → HP (G6A,
§3.19): `apply_damage_applied_v1` и `DamageHandler`. Это не generic
`EventApplierRegistry`/reducer — список выше остаётся в силе: post-G6A
abstraction review не нашёл достаточного evidence для generic mutation
abstraction (verdict: KEEP CONCRETE); Healing остаётся следующим meaningful
checkpoint для повторной оценки на основании фактически общего поведения и
различий concrete Damage/Healing implementations. EventStore, Event
