...
- **Closure assessment:** The foundation is sufficient for the implemented
  ...
  source provenance, Expertise/half proficiency, and Monster save/skill
  sources remain absent. Architecture §3.29 defines the effective
  weapon-proficiency membership and contribution semantics; TSK-0004
  implemented the persisted membership (`CharacterState.weapon_proficiencies`),
  and TSK-0012 implemented the production Attack-time contribution for the
  narrow Character Dagger consumer (`character_proficiency_bonus` applied to
  `AttackResult.proficiency_bonus` when the Dagger Definition ID is a member).
  Weapon-proficiency contribution for weapons beyond the Dagger remains
  unimplemented.
- **Broader unimplemented scope:** [DEF-0001](#def-0001),
  [DEF-0003](#def-0003), remaining Monster save/skill proficiency in
  [DEF-0004](#def-0004), and the remaining broader weapon-proficiency scope
  (beyond the narrow Character Dagger consumer) in [DEF-0011](#def-0011).
...
- **Closure assessment:** The slice is sufficient evidence for the Attack
  ...
  now implemented (TSK-0010). Section 3.32/DEC-0048 defines the exact
  Character Dagger Attack Result/Event and source-Damage boundaries;
  TSK-0012 implemented the production Character weapon path and reach
  validation, and TSK-0013 has since implemented its Attack→Damage→Monster
  HP continuation, confirmed through a real-adapter/filesystem round trip.
  Broader Monster actions, spells, other weapons, and broader consequences
  remain missing.
...
- **References:** [Architecture §3.17](ARCHITECTURE.md#317-minimal-phase-2-character-unarmed-attack-roll--monster-vertical-slice),
  ...
  [DEC-0045](DECISIONS.md#dec-0045--minimal-character-dagger-melee-targeting-uses-combat-owned-local-positions-and-a-narrow-5-foot-reach-policy),
  [Architecture §3.32](ARCHITECTURE.md#332-minimal-phase-3-character-dagger-attack-and-damage-contracts-tsk-0011),
  and [DEC-0048](DECISIONS.md#dec-0048--character-dagger-reuses-character-attackresult-and-adds-concrete-weapon-attackdamage-events).
...
- **Closure assessment:** This is sufficient for the first authoritative
  ...
  source orchestration and critical damage, and TSK-0013 has since
  implemented the narrow Character Dagger weapon/damage production
  continuation on top of §3.32/DEC-0048's exact source-Damage Result/Event,
  normal/critical/Finesse continuity, zero-source behavior, and unchanged
  `DamageApplied` V1 handoff, confirmed through a real-adapter/filesystem
  round trip. Broad Damage remains incomplete because typed defenses
  (resistance/immunity/vulnerability), temporary HP, and weapon/Monster
  damage sources beyond these two narrow consumers are absent.
...
- **References:** [Architecture §3.19](ARCHITECTURE.md#319-minimal-damage--hp-mutation-vertical-slice-g6a),
  ...
  [§3.29](ARCHITECTURE.md#329-minimal-authoritative-character-weapon-source-tsk-0001),
  [§3.32](ARCHITECTURE.md#332-minimal-phase-3-character-dagger-attack-and-damage-contracts-tsk-0011),
  [DEC-0033](DECISIONS.md#dec-0033--first-concrete-damage--hp-mutation-slice-g6a-stays-concrete),
...
- **Why deferred:** Zero-HP Attack eligibility for the current `AttackCommand`
  ...
  TSK-0016 (§3.34, DEC-0050) has since canonically defined the Character
  Death Save mechanic itself: triggering timing (an automatic in-memory
  consequence of `CombatStarted`/`TurnAdvanced`), Character-owned
  success/failure/stable/dead facts, natural 1/20 and three-success/
  three-failure outcomes, and its Result/Event/State-application boundaries.
  What remains deferred is the production continuation: no concrete
  resolver, Event builder/applier, Application handler wiring, or State
  schema V9 reader/writer exists yet.
- **Prerequisites:** The narrow Attack-eligibility part of DEF-0015 relevant to
  ...
  §3.31/DEC-0046. The combat timing/ownership contract this record asked for
  is now resolved by §3.34/DEC-0050, so no further architectural decision is
  a prerequisite for this narrow contract. This does not by itself authorize
  execution: per `docs/TASK.md`, `TSK-0017` depends on `TSK-0016` and stays
  `Backlog` until TSK-0016 reaches authoritative `Done` on `main`, and must
  then still pass its own separate refinement/readiness pass — forming exact
  `Scope`, `Out of scope`, `Acceptance criteria`, and `Verification` from the
  accepted §3.34 contract — before it may become `Ready`.
- **Planned approach:** Implement exactly the Character-specific mechanic
  §3.34/DEC-0050 already defines, rather than reusing
  `SavingThrowCommand(ability, dc)`: persist only the four Character
  death-save/lifecycle facts (`death_save_successes`, `death_save_failures`,
  `death_save_stable`, `dead`) on the existing `CharacterState` projection,
  through the two concrete Events (`CharacterDeathSaveResolved` V1,
  `CharacterDeathSaveFailureRecorded` V1) and their concrete pure typed
  Results/appliers §3.34 specifies. Do not require a universal Creature
  `LifeState`. Implementation is tracked as `TSK-0017` in `docs/TASK.md`.
- **Acceptance criteria:** Success/failure/critical outcomes, three-success and
  ...
  these are now canonically specified by §3.34/DEC-0050; this record closes
  `Done` only once TSK-0017 delivers them as tested production behavior
  (concrete resolvers/Results, Event builders/appliers, Application
  orchestration, and the State schema V9 reader/writer), not on contract
  definition alone.
- **References:** [Architecture §3.13](ARCHITECTURE.md#313-minimal-phase-2-character-saving-throw-vertical-slice),
...
- **History:** 2026-09-09 — TSK-0016 (§3.34, DEC-0050) canonically defined the
  minimal Character Death Save contract this record asked for: trigger,
  ownership, persisted facts, D20/Result/Event/State-application boundaries,
  and the target State schema V9. Status moves from `Deferred` to
  `In progress` because the architectural question is resolved and a
  concrete continuation task (`TSK-0017`) now exists in `docs/TASK.md`, not
  because the concern is `Done`: production implementation is still
...
- **Why deferred:** Architecture §3.29 defines the minimal authoritative
  ...
  production Character Dagger Attack consumer, and TSK-0013 has since
  implemented its Attack→Damage→Monster HP continuation (source-Damage
  Resolution, `CharacterWeaponAttackDamageResolved` V1, and optional
  `DamageApplied` V1/Monster HP application), confirmed through a
  real-adapter/filesystem round trip. Broader ranged/thrown/ammunition and
...
- **Prerequisites:** The prerequisite for the first Character Dagger consumer
  ...
  spatial State; TSK-0013 has since consumed that successful `AttackResult`
  and those authoritatively resolved weapon facts to implement the
  Attack→Damage→Monster HP continuation. Future ranged/ammunition/other-weapon
  consumers each still require their own source-specific prerequisites.
...
- **History:** 2026-08-30 — Created from the Phase 2 closure review; no
  ...
  `Deferred` for the broader Weapon attack scope. 2026-09-06 — TSK-0011
  (§3.32, DEC-0048) made the exact Character Dagger Attack Result/Event and
  source-Damage boundaries canonical without implementing them. The
  ...
  TSK-0013. Status stays `Deferred` for that broader scope. 2026-09-07 —
  TSK-0013 implemented the Character Dagger Attack→Damage→Monster HP
  continuation canonicalized by §3.32/DEC-0048 on top of TSK-0012's Attack
  Resolution: the pure `resolve_character_weapon_attack_damage` resolver and
  `CharacterWeaponAttackDamageResult`, the `CharacterWeaponAttackDamageResolved`
  V1 Event, and `AttackHandler` orchestration that reuses the unchanged
  `resolve_damage_amount`/`DamageResult`/`DamageApplied` V1/
  `apply_damage_applied_v1`/`replace_creature_in_snapshot` boundary for
  positive source damage only, confirmed through deterministic Domain/
  Application tests and a real-adapter (`FilesystemStateStore`,
  `PackagedDefinitionSource`, `PythonDiceEngine`) filesystem round trip. This
  closes the narrow Character Dagger part of this record's own scope
  entirely. Ranged/thrown/ammunition and other-weapon Attack/Damage behavior
  remain unimplemented. Status stays `Deferred` for that broader scope.
...
- **Why deferred:** The narrow Monster Scimitar Application consequence path
  ...
  persistence. Section 3.32/DEC-0048 defines the exact Character Dagger
  source-Damage contract: `CharacterWeaponAttackDamageResult`,
  ...
  handoff. TSK-0012 implemented the production Character Dagger Attack
  Resolution half of this chain, and TSK-0013 has since implemented the
  remaining source-Damage Resolution and Monster HP application continuation,
  confirmed through deterministic Domain/Application tests and a
  real-adapter/filesystem round trip. This closes the narrow Character
  Dagger consumer entirely; the broader Weapon attack/damage/critical
  continuation (other weapons, ranged/thrown/ammunition) remains deferred.
  Other Monster actions, resistance/immunity/vulnerability, and temporary HP
  are separately tracked by their own Roadmap/Deferred records and are not
  part of DEF-0013's own remaining scope.
- **Prerequisites:** For the first Character Dagger continuation, §3.32
  supplied the concrete damage-source and exact Event ordering/causation
  contract; TSK-0012 supplied the required successful `AttackResult` and
  authoritatively resolved weapon facts (`weapon_item_id`,
  `weapon_definition_id`, selected Ability/modifier), which TSK-0013
  orchestration has now consumed to implement that continuation. Future
  weapon consequence consumers (other weapons, ranged/thrown/ammunition)
  each still require their own source-specific prerequisites. DEF-0022 is
  required only if durability becomes part of a later slice.
- **Planned approach:** Implement the exact preserved chain from §3.32:
  ...
  emit ordered correlated source Events, and pass positive source damage
  through unchanged `DamageResult` / `DamageApplied` V1 without re-rolling in
  the applier.
...
- **History:** 2026-08-30 — Created from the Phase 2 closure review; no
  ...
  **not** supply this record's third prerequisite, an explicit Event
  ...
  2026-08-30 — Summary: G8 supplied the relevant Character-target part of
  DEF-0011 and a concrete Monster damage source (`MonsterAttackDefinition`'s
  `damage_dice`/`damage_modifier`/`damage_type`); G9 supplied the previously
  missing explicit Event ordering/causation design (§3.27, DEC-0042) —
  `MonsterAttackResolved → MonsterAttackDamageResolved → optional
  DamageApplied`. G9 Group 3 wired that already-accepted design into
  `AttackHandler`'s ordered emission and optional HP mutation, and G9 Group 4
  (§3.27) added real-adapter/filesystem integration evidence: a full
  production round trip through the real `FilesystemStateStore`,
  `PackagedDefinitionSource`, and `PythonDiceEngine` adapters confirms a
  fresh-reload-visible Character `current_hp` change, an untouched
  `CombatState` and unrelated Creature/Character projections, exactly one
  `save()` call, and the exact ordered/`causedBy` Event chain for a Goblin
  Scimitar attack. This closes out the narrow Goblin Scimitar
  Monster-consequence path entirely, using DEC-0042 as its accepted
  architectural rationale; Groups 3–4 implemented and verified that
  already-accepted decision without changing any contract, so no new
  Decision record was needed. DEF-0013 remains `Deferred` specifically for
  the unfinished broader Weapon attack/damage/critical continuation: a
  concrete Character Weapon damage source and its Attack→Damage application,
  the Equipment/Inventory ownership a Character Weapon attack requires,
  Character weapon proficiency, and the explicit (not auto-maximized) Finesse
  choice all remain unimplemented. Other Monster actions, Shortbow/range/
  reach, Multiattack, zero-HP action eligibility, active-turn Attack
  ...
  not auto-selected again. DEF-0013 remains `Deferred`. 2026-09-06 — TSK-0011
  (§3.32, DEC-0048) canonized the exact Character Dagger continuation as
  `Attack Resolution → source Damage Resolution → optional source-agnostic
  Damage Application`, including concrete source-Damage Result/Event,
  ...
  2026-09-07 — TSK-0013 implemented the remaining source-Damage Resolution
  and Monster HP application continuation canonicalized by §3.32/DEC-0048:
  the pure `resolve_character_weapon_attack_damage` resolver and
  `CharacterWeaponAttackDamageResult` (reusing the Attack-selected Ability/
  modifier without reselecting or maximizing it, and never adding
  proficiency to Damage), the `CharacterWeaponAttackDamageResolved` V1
  Event, and `AttackHandler` orchestration for all three miss/zero-source/
  positive-source branches — the positive branch reuses unchanged
  `resolve_damage_amount`/`DamageResult`/`build_damage_applied_from_attack_v1`/
  `apply_damage_applied_v1`/`replace_creature_in_snapshot` and performs
  exactly one `StateStore.save()`. Confirmed through deterministic Domain/
  Application tests and a real-adapter (`FilesystemStateStore`,
  `PackagedDefinitionSource`, `PythonDiceEngine`) filesystem round trip
  showing the exact `CharacterWeaponAttackResolved → CharacterWeaponAttackDamageResolved
  → DamageApplied` Event chain and causation, and Goblin `current_hp`
  durably changed after a fresh reload. This closes the narrow Character
  Dagger Attack→Damage→Monster HP consequence entirely, using DEC-0042/
  DEC-0048 as its accepted architectural rationale; no new Decision record
  was needed. DEF-0013 remains `Deferred` specifically for the unfinished
  broader Weapon attack/damage/critical continuation: other weapons and
  ranged/thrown/ammunition remain unimplemented. Other Monster actions
  beyond the Goblin Scimitar, resistance/immunity/vulnerability, and
  temporary HP are not part of DEF-0013's own remaining scope and are
  separately tracked by their own Roadmap/Deferred records. DEF-0013's
...
- **History:** 2026-09-09 — TSK-0016 (§3.34, DEC-0050) canonically defined the
  Character zero-HP turn / Death Save slice this record coordinated with
  DEF-0005: trigger timing, Character-owned persisted facts, and the
  Damage-at-zero/ordinary-Healing interaction for that one narrow mechanic.
  DEF-0015 stays `Deferred`: this closes only that narrow coordinated
  Character slice (production implementation still pending TSK-0017, tracked
  by DEF-0005). Targetability of a creature already at zero HP, Monster
  death/stabilization/lifecycle policy and Monster Death Saves, broader
  Healing-recovery semantics (rest recovery, external stabilization,
