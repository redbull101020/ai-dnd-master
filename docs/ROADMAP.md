...
It changed no production code, no `CombatState`, and no existing
...
DEFINED (§3.35, DEC-0051) AND IMPLEMENTED (TSK-0019):
    ...
    actor-first validation, then active-Combat existence/id-match, reusing
        the existing ENTITY_NOT_FOUND shape (no new ErrorCode)
    pure EndCombatResult(combat_id) and CombatEnded V1 (combatId only,
        causedBy: null)
    apply_combat_ended_v1 returning the replacement Combat projection
        (None), consumed by Application exactly like every other Combat
        applier's returned CombatState
    StateSnapshot.combat returns to None; no State schema version change
        (V9 already serializes combat=None)
    Combat-owned transient State (order/active_index/round/positions/
...
DEFINED (§3.36, DEC-0052) AND IMPLEMENTED (TSK-0022):
    ...
    one Command places exactly one not-yet-positioned participant; already
        positioned creature_id is rejected (not reposition/teleport/Movement)
    actor-first validation, then active-Combat existence/id-match, then
        placement-subject lookup, then combat.order membership, then
        no-existing-position check; reuses ENTITY_NOT_FOUND/
        ACTION_NOT_AVAILABLE (no new ErrorCode)
    pure PlaceCombatantResult(combat_id, creature_id, x, y) and
        CombatantPlaced V1 (combatId/creatureId/x/y, causedBy: null)
    apply_combatant_placed_v1 appends exactly one new CombatPosition to the
        existing CombatState.positions tuple; every other Combat field
        (id/round/order/active_index/action_spent) and all Creature/
        Character/Inventory/Equipment State unchanged
...
STILL UNDESIGNED (voluntary Movement, later separately evidenced consumer):
    ...
    reposition, Dash, Disengage, forced movement
    difficult terrain, pathfinding, collision, occupancy/footprint, elevation
