...
* [ ] Turn/action economy and turn resources (actions, bonus actions, reactions budget per turn) — §3.28/DEC-0043 define and now implement the canonical active-turn eligibility boundary for the currently supported `AttackCommand` paths (row above, TSK-0006); §3.33/DEC-0049 (TSK-0014) now canonically defines, as a decision only, the first narrow ordinary-Action resource contract for those same three currently implemented `AttackCommand` consumers (`CombatState.action_spent`, `TurnActionSpent` V1, planned State schema V8) — production implementation remains a later task. Bonus Action and Reaction resources, Movement, Extra Attack, Action Surge, Multiattack, per-turn resets beyond this narrow slice, and the broader action economy remain open.
